...
def render_source_button(file_path, button_key):
    """Extracts the filename from a path and creates a secure download/view button."""
    if not file_path or not os.path.exists(file_path):
        st.caption("⚠️ Source file path not found or inaccessible.")
        return

    # Extract just the filename (e.g., "Safety_Protocols.pdf")
    ...
    mime_type = "application/pdf" if ext == "pdf" else "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    if ext == "csv": 
        mime_type = "text/csv"

    # Create a native Streamlit download button styled like a citation link
    st.download_button(
        label=f"📄 Open Source: {file_name}",
        data=file_bytes,
        file_name=file_name,
        mime=mime_type,
        key=button_key,
        help="Click to download or view the exact document the AI referenced."
    )
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("Generating answer. This may take some time..."):
            ...
            async def get_ai_response():
                # We use 'ainvoke' (async invoke) to keep the app responsive
            ...
            if context_docs:
                with st.expander("View Source Documents"):
                    for i, doc in enumerate(context_docs):
                        # Safely grab the file path from the metadata
                        source_path = doc.metadata.get("source", "")
                        
                        st.markdown(f"**Source {i+1}:**")
                        
                        # 1. Trigger the clickable download button using your helper function
                        if source_path:
                            render_source_button(source_path, button_key=f"dl_btn_{i}")
                             
                        # 2. Show the text snippet it read
